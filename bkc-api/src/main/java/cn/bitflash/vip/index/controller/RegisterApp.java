...
public class RegisterApp {
    ...
    @GetMapping("registerWeb2/{mobile}/{pwd}/{invitationCode}")
    public R registerWeb2(@PathVariable String mobile, @PathVariable String pwd,
                         @PathVariable String invitationCode, HttpServletResponse response) {
        response.addHeader("Access-Control-Allow-Origin", "*");
        UserSecretEntity oldUser = indexFeign.selectUserLoginEntityByMobile(mobile);
        if (oldUser != null) {
            return R.error(501, "手机号已经存在");
